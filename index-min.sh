#!/bin/bash
get_execution_directory(){ read -p "Enter the directory where the script will be executed: " execution_dir;[ ! -d "$execution_dir" ] && echo "Error: The specified directory does not exist. Aborting." && exit 1;cd "$execution_dir"||exit 1; }
rar_info=()
gather_rar_files(){ local rar_files=(*.rar); echo "List of RAR files:"; for rar_file in "${rar_files[@]}"; do echo "$rar_file"; done; }
manage_contents_folder(){ local contents_folder="Contents"; [ -d "$contents_folder" ] && echo "Deleting existing \"$contents_folder\" folder..." && rm -rf "$contents_folder"; echo "Creating \"$contents_folder\" folder..."; mkdir "$contents_folder"; }
